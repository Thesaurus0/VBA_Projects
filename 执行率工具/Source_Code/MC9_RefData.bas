...
'    Dim dictColIndex As Dictionary
'
'    Call fReadSheetDataByConfig("PRODUCT_MASTER", dictColIndex, arrData, , , , , shtPurchaseODRaw)
'    Call fValidateDuplicateInArray(arrData, Array(dictColIndex("ProductProducer"), dictColIndex("ProductName"), dictColIndex("ProductSeries")), False, shtPurchaseODRaw, 1, 1, "厂家 + 名称 + 规格")
'
...
'    If dictMiscConfig Is Nothing Then Call fReadMiscConfig2Dictionary
'
'    If Not dictMiscConfig.Exists("FIRST_LEVEL_COMMISSION_DEFAULT") Then fErr "配置没有设置采芝林默认配送费：FIRST_LEVEL_COMMISSION_DEFAULT"
'    fGetConfigFirstLevelDefaultComm = dictMiscConfig("FIRST_LEVEL_COMMISSION_DEFAULT")
'End Function
'
...
'
'    dblDefault = dictMiscConfig("SECOND_LEVEL_COMMISSION_DEFAULT_" & sCompID)
'
'    If sCompID = "CZL" Then
...
'End Function
'
Function fReadMiscConfig2Dictionary()
    Dim asTag As String
    Dim arrColsName(3)
    ...
    For lEachRow = LBound(arrConfigData, 1) To UBound(arrConfigData, 1)
        ...
        If sValueType = "GET_VALUE" Then
            dictMiscConfig.Add sKey, arrConfigData(lEachRow, arrColsIndex(2))
        ElseIf sValueType = "GET_ADDRESS" Then
            dictMiscConfig.Add sKey, shtSysConf.Cells(lActualRow, lConfigStartCol + arrColsIndex(2) - 1).Address(external:=True)
        Else
...
Function fGetSysMiscConfig(sSettingItemID As String, Optional sMsgHeader As String = "")
    ...
    If Not dictMiscConfig.Exists(sSettingItemID) Then
        fErr "[System Misc Settings] has not such config item: " & sSettingItemID & vbCr & vbCr & sMsgHeader
    End If

    fGetSysMiscConfig = dictMiscConfig(sSettingItemID)
End Function
'
...
''    Dim sKey As String
'
'    Call fRemoveFilterForSheet(shtPurchaseODRaw)
'
''    If dictProductMaster Is Nothing Then Call fReadSheetProductMaster2Dictionary
'
