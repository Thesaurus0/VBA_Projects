...
Function fGetFilesFromFolder(sFolder As String)
    Dim arrFiles()
    ...
    If oFolder.Files.Count <= 0 Then GoTo exit_fun
    
    ReDim arrFiles(1 To oFolder.Files.Count)
    
    i = 0
     
    For Each eachFile In oFolder.Files
        i = i + 1
        arrFiles(i) = eachFile.Path
    Next
